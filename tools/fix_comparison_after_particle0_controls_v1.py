...
def main():
    ...
    for r in rows:
        if r.get('comparison_id')=='C1_STREAM_RELATION_VS_MISSING_CONTROL':
            ...
            r['missing_control']='residual/class-specific/heldout controls' if order_ok else 'order/residual/class-specific/heldout controls'
            r['risk']='particle0 is causally important; order-index artifact is reduced; known-observable proxy and class-specific bias remain'
            r['recommended_next_experiment']='run known-observable residual, class-specific all-head gradients, and per-file heldout stability'
            r['status']='SUPPORTED_BY_TARGETED_CONTROL_AND_ORDER_CONTROL_NEEDS_RESIDUAL'
            r['training_label']='particle0_control_and_order_supported_residual_required'
            r['evidence_json']=json.dumps({'remove_particle0':rem0,'keep_only_particle0':keep0,'random_remove1_acc_drop_mean':rand_drop,'random_remove1_flip_mean':rand_flip,'target_vs_random_drop_ratio':ratio,'order_max_random_flip':max_order_flip,'confusion_atlas_available':confusion_ok},ensure_ascii=False)
        if r.get('comparison_id')=='C14_ORDERING_VS_PHYSICAL_COORDINATE' and order_ok:
            ...
            r['missing_control']='known-observable residual / heldout'
            r['risk']='array-index shortcut risk is strongly reduced, but leading-pT/known-observable proxy remains possible'
            r['recommended_next_experiment']='run known-observable residual and per-file heldout; optionally inspect max-logit-diff outliers'
            r['status']='ORDER_INDEX_ARTIFACT_REDUCED_NEEDS_RESIDUAL_HELDOUT'
            r['training_label']='order_artifact_reduced_residual_heldout_required'
    ...
    dataset=[]
    for r in rows:
        dataset.append({'schema':'automatic_comparison_training_row.v2.fixed_after_particle0_order_confusion','input':{k:r.get(k,'') for k in ['comparison_id','claim','support_signal','contradiction_signal','missing_control','risk','evidence_json']},'target':{k:r.get(k,'') for k in ['status','priority','training_label','recommended_next_experiment']}})
    wjsonl(args.dataset,dataset)
    out={'schema':'automatic_comparison_engine.v2','generated_at_utc':datetime.now(timezone.utc).isoformat(),'postprocess':'particle0_order_confusion_fix_v1','comparisons':rows,'p0':[r for r in rows if r.get('priority')=='P0'],'next_big_stream_policy':'controls_then_staged_large_stream'}
    wjson(args.json,out)
    md=['# Automatic Comparison Engine v2\n\n',
        'v2 after particle0/top-k controls and order/confusion controls.\n\n',
        '## P0 comparisons\n',
        ...
        'Interpretation: particle0/core is targeted-causal and array-index artifact is reduced. Remaining P0: known-observable residual, class-specific gradients, heldout/per-file stability.\n\n',
        '## Files\n\n',
        f'- JSON: `{args.json}`\n',
        f'- CSV: `{args.rows}`\n',
    ...
    print(json.dumps({'ok':True,'patched_C1':True,'patched_C14':order_ok,'target_vs_random_drop_ratio':ratio,'order_max_random_flip':max_order_flip,'out_json':args.json},indent=2,ensure_ascii=False))
