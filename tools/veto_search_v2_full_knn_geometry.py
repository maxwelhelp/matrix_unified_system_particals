...
def pairwise(vals):
    ...
    for i in range(len(vals)):
        for j in range(i+1,len(vals)):
            out.append(dist(vals[i],vals[j]))
    return out
def summarize_knn(prefix,ei,center_idx,nb,feat,points,pt,hard_pt_quantile=0.7):
    ...
    dr_center=[dist(c,center) for c in coords]
    pdr=pairwise(coords)
    ...
    out={
        ...
        prefix+'_knn_deltaR_to_center_mean':sum(dr_center)/len(dr_center) if dr_center else 0.0,
        prefix+'_knn_deltaR_to_center_min':min(dr_center) if dr_center else 0.0,
    ...
    # PID fractions by neighbor type
    for j,nm in [(6,'charged_hadron'),(7,'neutral_hadron'),(8,'photon'),(9,'electron'),(10,'muon')]:
        if feat.shape[1]>j and nb:
            out[prefix+'_knn_'+nm+'_frac']=float(feat[ei,j,nb].float().mean())
        else:
            out[prefix+'_knn_'+nm+'_frac']=0.0
    return out
def event_features(ei,group,phase_row,batch,local_knn_idx,patch_k,p0_lep_thr,hard_pt_quantile=0.7):
    feat=batch['features']; points=batch['points']; pt=pt_values(batch); mask=real_mask(batch)
    p0=0
    # local_knn_idx shape is [1,N,K] because we computed KNN for a single event.
    ...
    p0c=points[ei,:,p0].float().cpu(); lepc=points[ei,:,lep_idx].float().cpu()
    p0_lep_dr=dist(p0c,lepc)
    p0_aligned=int(p0_lep_dr < p0_lep_thr)
    had_all=[j for j in range(pt.shape[1]) if bool(mask[ei,j]) and is_had(feat,ei,j)]
    had_drs=[]
    for j in had_all:
        hc=points[ei,:,j].float().cpu(); had_drs.append((dist(hc,lepc),j,float(pt[ei,j])))
    ...
    row={
        ...
        'p0_is_lepton_aligned':p0_aligned,
    ...
    row.update(summarize_knn('p0',ei,p0,p0_nb,feat,points,pt,hard_pt_quantile))
    row.update(summarize_knn('lep',ei,lep_idx,lep_nb,feat,points,pt,hard_pt_quantile))
    return row
...
def main():
    ...
    with torch.no_grad():
        for ei,(phase_row,g) in wanted.items():
            local_idx=knn(pts[ei:ei+1],a.patch_k).detach().cpu()
            rows.append(event_features(ei,g,phase_row,batch,local_idx,a.patch_k,a.p0_lepton_dr_threshold))
    ...
    out={'ok':True,'protected_highiso_n':len(protected),'actual_confused_highiso_n':len(confused),'rows':len(rows),'p0_lepton_dr_threshold':a.p0_lepton_dr_threshold,'top_contrasts':contrasts[:30]}; wjson(a.out_json,out)
