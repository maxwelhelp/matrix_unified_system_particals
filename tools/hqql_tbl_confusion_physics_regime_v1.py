...
def main():
    ...
    for ei in range(len(y)):
        ...
        # lepton candidate = highest-pt electron/muon if exists.
    ...
    for g,xs in groups.items():
        ...
        summ.append(row)
    wcsv(a.out_events,rows); wcsv(a.out_summary,summ); wjson(a.out_json,{'ok':True,'events':len(rows),'groups':{k:len(v) for k,v in groups.items()},'k':k,'missing':list(res.missing_keys),'unexpected':list(res.unexpected_keys)})
    ...
    print(json.dumps({'ok':True,'events':len(rows),'groups':{k:len(v) for k,v in groups.items()},'out_md':a.out_md},indent=2,ensure_ascii=False))
