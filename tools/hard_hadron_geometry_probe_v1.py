...
def summarize(rows):
    ...
    out = []
    for (group, role), xs in sorted(by.items()):
        n = len(xs)
        out.append({
            'group': group,
            'particle_role': role,
...
def main():
    ...
    rows = readcsv(args.annotated_top_particles)
    if not rows:
        raise RuntimeError('Missing annotated top particles. Run INTERNAL_ACTIVATION_CONTRAST_V2_1 first.')
    ...
    pt = pt_values(batch).detach().cpu()

    from weaver.nn.model.ParticleNet import knn

    # Map L2 top3 by event/particle.
    l2_top = {}
    for r in rows:
        if r.get('head_id') == args.l2_head:
            key = (int(fnum(r.get('event_idx'), -1)), int(fnum(r.get('particle_idx'), -1)))
            l2_top[key] = r

    out = []
    knn_cache = {}
    candidates = [r for r in rows if r.get('head_id') == args.focus_head and is_had_role(r.get('particle_role',''))]
    for r in candidates:
        ei = int(fnum(r.get('event_idx'), -1)); pi = int(fnum(r.get('particle_idx'), -1))
        if ei < 0 or pi < 0 or ei >= points.shape[0]: continue
        best = r.get('best_lepton_idx', '')
        if best == '': continue
        best = int(fnum(best, -1))
        if best < 0: continue
        if ei not in knn_cache:
            with torch.no_grad():
                knn_cache[ei] = knn(points[ei:ei+1], args.knn_k).detach().cpu()
        nb = [int(x) for x in knn_cache[ei][0, best].tolist() if int(x) != best]
        is_in = int(pi in nb)
        rank = nb.index(pi) + 1 if pi in nb else 99
        dr = dist(points[ei,:,pi].float(), points[ei,:,best].float())
        l2r = l2_top.get((ei, pi))
        out.append({
            'event_idx': ei,
            'group': r.get('group'),
            'focus_head': args.focus_head,
            'particle_idx': pi,
            'particle_role': r.get('particle_role'),
            'pid': r.get('pid'),
            'particle_pt': r.get('pt'),
            'l1_rank': r.get('rank'),
            'l1_activation_norm': r.get('particle_activation_norm'),
            'best_lepton_idx': best,
            'deltaR_to_best_lepton': dr,
            'is_in_best_lepton_knn': is_in,
            'best_lepton_knn_rank': rank,
            'same_particle_in_L2_ch128_top3': int(l2r is not None),
    ...
    summ = summarize(out)
    wcsv(args.out_rows, out); wcsv(args.out_summary, summ)
    wjson(args.out_json, {'ok': True, 'rows': len(out), 'focus_head': args.focus_head, 'l2_head': args.l2_head, 'summary': summ})

    md = ['# HARD_HADRON_GEOMETRY_PROBE_V1\n\n',
          'Probe for hard-hadron topology-confound in `L1_ch80:96`. It checks whether hard hadron top activators enter the best-lepton KNN, their deltaR to the lepton, and whether the same particle also appears in `L2_ch128:160` top activations.\n\n',
          '## Summary\n',
          mdtab(['group','role','n','deltaR_lep','in_lep_KNN','knn_rank','pt','L2_same_particle_top3','L1_act','L2_act_if_top3'], [[s['group'], s['particle_role'], s['n'], fmt(s['mean_deltaR_to_best_lepton']), fmt(s['in_best_lepton_knn_rate']), fmt(s['mean_best_lepton_knn_rank']), fmt(s['mean_particle_pt']), fmt(s['l2_same_particle_top3_rate']), fmt(s['mean_l1_activation_norm']), fmt(s['mean_l2_activation_norm_if_top3'])] for s in summ]),
          '\n## Interpretation\n\nIf B_confused hard hadrons have lower deltaR to best_lepton and higher in_lep_KNN rate than A, then the L1 route is driven by lepton-neighborhood hadronic contamination. If the same particles do not appear in L2_ch128:160 top activations, then L2 does not recover Hqql evidence from that hard-hadron context.\n']
    Path(args.out_md).parent.mkdir(parents=True, exist_ok=True)
    Path(args.out_md).write_text(''.join(md), encoding='utf-8')
    print(json.dumps({'ok': True, 'rows': len(out), 'out_md': args.out_md}, indent=2))
