...
| P0 | HIGH | 0.9750 | T4_HQQL_TBL_SIGNATURE | Is the current stream dominated by Hqql/Tbl high-confidence events? | Run class-specific all-head gradients: Hqql, Tbl, Tbqq, Wqq, Zqq. |
| P0 | HIGH | 0.9450 | T2_HEAD_RANK_STABILITY | Do the same heads stay important across snapshots? | Keep tracking; if unstable, split by class/sample size and run heldout stability. |
...
| P1 | LOW | 0.1950 | T6_WIDE_SECONDARY_CONTEXT | Besides particle0/core, is there stable wide/secondary particle context? | Run route-neighbor trace for top all-head particles and wide secondary particles. |
...
- **T4_HQQL_TBL_SIGNATURE** is `HIGH` (0.9750). Next: Run class-specific all-head gradients: Hqql, Tbl, Tbqq, Wqq, Zqq.
- **T2_HEAD_RANK_STABILITY** is `HIGH` (0.9450). Next: Keep tracking; if unstable, split by class/sample size and run heldout stability.
