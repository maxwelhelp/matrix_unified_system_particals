...
| P0 | NEEDS_HELDOUT | C13_PER_FILE_HELDOUT_STABILITY | snapshots=11 but no per-file heldout breakdown | same extracted tiny files can fake stable patterns |
| P0 | NEEDS_ORDER_CONTROL | C14_ORDERING_VS_PHYSICAL_COORDINATE | particle0_edge_signal=0.986864441102955 support=120 | particle index can encode sorting by pt, not a physical interaction |
...
| P0 | RUN_SAMPLED_LARGE_STREAM_BUT_PRIORITIZE_CONTROLS | C16_BIG_STREAM_READINESS | current_snapshots=11 stream_events=4871 | big data can make wrong shortcut look very confident |
| P0 | CANDIDATE_STRONG_MISSING_CONTROL | C1_STREAM_RELATION_VS_MISSING_CONTROL | relation_signal=1.0 support=153 | could be sorting shortcut or normal leading-particle bias |
| P0 | NEEDS_ROUTE_TRACE | C2_WIDE_PATTERN_VS_ROUTE_TRACE | relation_signal=0.7261321500647455 support=30 dst=hypothesis:T6_WIDE_SECONDARY_CONTEXT | wide relation can be class imbalance, loose fragments, or sorting artifact |
...
| P0 | SCALING_PARTIAL | C7_SAMPLE_SIZE_SCALING | distinct_n_events=[640, 2560, 5120] snapshots=11 | large stream can amplify shortcuts if controls are missing |
...
| P1 | NEEDS_CLASS_SPECIFIC_TEST | C3_PATCH_VS_GRADIENT_DIVERGENCE | divergent_heads=12 top=[{'head_id': 'L1_ch112:128', 'gate_abs_grad': 0.842360487754923, 'patch_acc_drop': 0.0, 'role': 'middle learned-neighborhood / route-composition head'}, {'head_id': 'L1_ch16:32', 'gate_abs_grad': 0.7222795635927468, 'patch_acc_drop': 0.0, 'role': 'middle learned-neighborhood / route-composition head'}, {'head_id': 'L0_ch40:48', 'gate_abs_grad': 0.7015100818593057, 'patch_acc_drop': 0.0, 'role': 'early feature/geometry/PID reader'}] | gradient support is local; patch may reveal redundancy or compensation |
| P1 | NEEDS_HELDOUT | C4_HEAD_RANK_STABILITY_VS_RUN_CHANGES | stability_score=0.9400 top_heads=['L1_ch112:128', 'L0_ch40:48', 'L2_ch224:256', 'L1_ch16:32', 'L0_ch48:56'] | repeated same data can fake stability |
